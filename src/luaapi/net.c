...
static int l_net_load(lua_State* state)
{
    ...
    if (strcmp(ip_version, "ipv6") == 0)
    {
        ...
        net_init(&moduleData.server_ipv4, address, port, ip_version);
    }
    else
    {
        moduleData.is_ipv4 = true;
        net_init(&moduleData.server_ipv6, address, port, ip_version);
    }
...
static int l_net_bind_socket(lua_State* state)
{
    ...
    if (moduleData.is_ipv4)
        err = net_bind_socket(&moduleData.server_ipv4, moduleData.socket);
    else
        err = net_bind_socket(&moduleData.server_ipv6, moduleData.socket);
...
static int l_net_listen(lua_State* state)
{
    ...
    check_socket(moduleData.socket, "listen");
...
static int l_net_accept(lua_State* state)
{
    ...
    l_net_check_socket(moduleData.accept, "recieve");

    if (moduleData.is_ipv4)
    {
        int temp_socket = net_accept_connection(&dest_ipv4, moduleData.socket);
        lua_pushinteger(state, temp_socket);
    } else {
        int temp_socket = net_accept_connection(&dest_ipv6, moduleData.socket);
        lua_pushinteger(state, temp_socket);
    }
...
static int l_net_send(lua_State* state)
{
    ...
    char* data = l_tools_toStringOrError(state, 2);
    int len  = l_tools_toIntegerOrError(state, 3);

    int size = net_send_data(temp_socket, data, len);
